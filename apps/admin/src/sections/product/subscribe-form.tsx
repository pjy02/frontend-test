...
import { CreditCard, LoaderCircle, Server, Settings } from "lucide-react";
...
export default function SubscribeForm<T extends Record<string, any>>({
  ...
  return (
    <DetailSheet
      description={t(
        "form.editorDescription",
        "Configure presentation, billing rules, quotas, and the infrastructure available to this plan."
      )}
      footer={
        <StickyActions
          description={t(
            "form.saveDescription",
            "New products remain hidden and unavailable for sale until explicitly enabled."
          )}
        >
          <Button
            disabled={loading}
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)}>
          <Tabs className="w-full" defaultValue="basic">
            <TabsList className="mb-6 grid w-full grid-cols-3">
              ...
              <TabsTrigger className="flex items-center gap-2" value="servers">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                          <FormLabel>
                            {t("form.showOriginalPrice", "Show Original Price")}
                          </FormLabel>
                          <FormDescription>
                            {t(
